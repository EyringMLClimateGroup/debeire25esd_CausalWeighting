...
import time
import glob
from mpi4py import MPI  # if ERROR https://stackoverflow.com/questions/36156822/error-when-starting-open-mpi-in-mpi-init-via-python
import numpy
import os, sys, time
...
model_name = sorted([
    os.path.basename(file_path) for file_path in glob.glob(config["model_files_era5"])
])
model_path = config["model_path_era5"]
...
for model in model_name[first_model_idx:last_model_idx]:
    for method_arg in ['pcmci']:
        ...
        if "[12, 1, 2]" in model:
            selected_comps_file="./selected_comps_files_1979-2014_timebin1/selected_comps_era5_djf.csv"
        elif "[3, 4, 5]" in model:
            selected_comps_file="./selected_comps_files_1979-2014_timebin1/selected_comps_era5_mam.csv"
        elif "[6, 7, 8]" in model:
            selected_comps_file="./selected_comps_files_1979-2014_timebin1/selected_comps_era5_jja.csv"
        elif "[9, 10, 11]" in model:
            selected_comps_file="./selected_comps_files_1979-2014_timebin1/selected_comps_era5_son.csv"
        else : continue
        ...
        file_name = config["pcmci_results_era5"]+'/results_%s_%d-VAR_%d-LAG_%s.bin' % (
