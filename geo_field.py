...
class GeoField:
    ...
    def return_masked_months(self, month_mask):
        ...
        if self.use_cdftime:
            ndx =list(filter(lambda i: self.cdftime.num2date(tm[i]).month in month_mask, range(len(tm))))
        else:
            ndx = list(filter(lambda i: date.fromordinal(int(tm[i])).month in month_mask, range(len(tm))))
              
        # print ndx
        # self.tm = tm[ndx]
        time_mask = np.ones(len(self.tm), dtype='bool')
        for i in ndx : time_mask[i] = False
